...
public class ArrayIndexedCollection<T> implements List<T>{
	...
	private ArrayIndexedCollection( Collection<? extends T> other, boolean checkOther, int initialCapacity){
	...
	private void doubleInSize() {
		elements = (T[]) Arrays.copyOf(elements, size*2);
	}
	/**
	...
	public Object[] toArray() {
		...
		return (Object[]) Arrays.copyOf( elements, size);
	}
	...
	private static class ArrayElementsGetter<T> implements ElementsGetter<T>{
		 ...
		 */
		private ArrayElementsGetter( ArrayIndexedCollection<T> arr) {
			this.arr = arr;
			currIndex = 0;
